...
for n in runs[rank::size]:
    ...
    with HDF5Output(f"myoutput_{n}_fm.h5") as o:
        ...
        spectrum_output = model_binner.generate_spectrum_output(
            model_result, OutputSize.light
        )

        # Collect contributions
        spectrum_output["Contributions"] = store_contributions(
            model_binner, model, output_size=OutputSize.lighter
        )
