...
def parse_keywords(keywords):
    ...
    if keywords in ("contribs",):
        ...
        table = [
            (
                " / ".join(c.input_keywords()),
                f"{c.__name__}",
                c.__module__.split(".")[0].split("_")[-1],
            )
            for c in cf.chemistryKlasses
            if hasattr(c, "input_keywords")
        ]
        print(
            tabulate.tabulate(
...
def show_parameters(model):
    ...
    short_desc = []
    for v in model.fittingParameters.values():
        doc = v[2].__doc__
    ...
    short_desc = []
    for v in model.derivedParameters.values():
        doc = v[2].__doc__
        if doc is None or doc == "None":
            short_desc.append("")
        else:
...
def show_plugins():
    ...
    print("---------------------------")
    for k in successful_plugins.keys():
        print(k)
...
def main():  # noqa: C901
    ...
    if GlobalCache()["xsec_float32"]:
        root_logger.info("Float32 mode enabled: cross-sections and computation arrays use float32 precision")
    ...
    if args.output_file:
        ...
        with HDF5Output(args.output_file, append=True) as o:
            ...
            spectrum = binning.generate_spectrum_output(result, output_size=output_size)
