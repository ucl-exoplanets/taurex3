"""Utility functions for spectrum."""

import numpy as np


def voigt_wolfz(nu, alpha, gamma):
    """Return the Voigt line shape at x.

    with Lorentzian component HWHM gamma
...
def compute_voigt(
    ...
    mask_trans, mask_grid_flat = np.where(
        (xgrid[start_wn[:, None]] <= xgrid) & (xgrid[end_wn[:, None]] >= xgrid)
    )
    ...
    for idx, st, ed in zip(uniq[:-1], index[:-1], index[1:], strict=True):
