...
pickle_test_data = {
    ...
    "comments": [
        ...
        "C. Richard, I.E. Gordon, L.S. Rothman, M. Abel, "
        "L. Frommhold, M. Gustafsson, et al, "
        "JQSRT 113, 1276-1285 (2012).",
...
class PickleCIATest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.pop = None
        with patch("builtins.open", mock_open(read_data=self.data)):
    ...
    def test_properties(self):
        ...
        np.testing.assert_equal(
            pickle_test_data["t"],
            self.pop.temperatureGrid,
        )
        np.testing.assert_equal(
            pickle_test_data["wno"],
            self.pop.wavenumberGrid,
        )
        np.testing.assert_equal(
            pickle_test_data["xsecarr"],
            self.pop._xsec_grid,
        )

    def test_pathname_naming(self):
        """Test pathname naming."""
        with patch("builtins.open", mock_open(read_data=self.data)):
            pop = PickleCIA("/unittestfile/HeH-2HF.db")
            # We cannot assert on the mock file call since
            # mock_open does not handle context manager well
            self.assertEqual(pop.pairName, "HeH-2HF")

    def test_cia_calc(self):
        """Test CIA calculation."""
        np.testing.assert_allclose(
            self.pop.cia(225),
            pickle_test_data["xsecarr"][1],
        )

    def test_max_min_temps(self):
        """Test max min temps."""
        np.testing.assert_equal(
            self.pop.cia(100000000),
            self.pop._xsec_grid[-1],
        )
        np.testing.assert_equal(
            self.pop.cia(0.0000001),
            self.pop._xsec_grid[0],
        )
...
class HitranCIATest(unittest.TestCase):
    ...
    def test_reads_problematic_headers(self):
        """Test reads problematic headers."""
        test_cases = [
            (
            ...
            (
                (
                    "eq-H2 -- eq-H2 0.250000 2400.0000 8 "
                    "40.000 3.683e-44 -.999 34\n"
                    "0.2500 4.706E-51\n"
                    "0.5000 1.957E-50\n"
                    "0.7500 4.401E-50\n"
                    "1.0000 7.983E-50\n"
        ...
        for (
            ...
            with self.subTest(header=hitran_data.splitlines()[0]):
                with patch(
                    "builtins.open",
                    mock_open(read_data=hitran_data),
                ):
